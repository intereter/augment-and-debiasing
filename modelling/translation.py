...
from .model_my import MBartForConditionalGeneration_my
from .utils_my import get_random_mask,get_random_mask_feature
import math
...
class TranslationNetwork_my(torch.nn.Module):
    def __init__(self, input_type, cfg, task, mycfg=None) -> None:
        super().__init__()
        self.cfg = cfg
        self.logger = get_logger()
        self.task = task
        ...
        if 'pretrained_model_name_or_path' in cfg:
            ...
            self.model = MBartForConditionalGeneration_my.from_pretrained(
                cfg['pretrained_model_name_or_path'],
                **cfg.get('overwrite_cfg', {}) 
            )
            self.mycfg = mycfg
            if mycfg is not None and mycfg.get('debias_lm_head',None) is not None:
                head_num = mycfg['debias_lm_head']['head_num']
                self.model.multi_lm_head = []
                self.model.mycfg = mycfg
                for _ in range(0,head_num):
                    temp_mlp = nn.Linear(self.model.lm_head.in_features,self.model.lm_head.out_features, bias=False)
                    temp_mlp.to('cuda')
                    with torch.no_grad():
                        temp_mlp.load_state_dict(self.model.lm_head.state_dict())
                    
                        self.model.multi_lm_head.append(temp_mlp)
            self.model.cfg = cfg
        elif 'model_config' in cfg:
            self.logger.info('Train translation network from scratch using config={}'.format(cfg['model_config']))
            config = MBartConfig.from_pretrained(cfg['model_config'])
        ...
        self.model.model.if_eos_index = self.cfg.get('if_eos_index',True)
        ...
        if self.task in ['S2T', 'G2T','S2T_RGBInput'] and 'pretrained_model_name_or_path' in cfg:
            #in both S2T or G2T, we need gloss_tokenizer and gloss_embedding
            self.gloss_tokenizer = GlossTokenizer_G2T(tokenizer_cfg=cfg['GlossTokenizer'])
            self.gloss_embedding = self.build_gloss_embedding(**cfg['GlossEmbedding'])
            #debug
    ...
    def prepare_feature_inputs(self, input_feature, input_lengths, gloss_embedding=None, gloss_lengths=None):
        if self.task == 'S2T_glsfree':
            ...
            if self.gls_eos=='gls':
                ...
                suffix_embedding = [self.model.model.shared.weight[self.text_tokenizer.eos_index,:]]
            if self.task in ['S2T', 'G2T','S2T_RGBInput'] and self.gloss_embedding:
                if self.gls_eos == 'gls':
                    src_lang_code_embedding = self.gloss_embedding.weight[ \
                        self.gloss_tokenizer.convert_tokens_to_ids(self.gloss_tokenizer.src_lang),:] #to-debug
                else:
        ...
        for ii, feature in enumerate(input_feature):
            ...
            if feature_w_suffix.shape[0]<max_length:
                ...
                inputs_embeds.append(feature_w_suffix)
            attention_mask[ii, :valid_len+suffix_len] = 1
        real_lenghts = input_lengths.clone().detach()
        for _ in range(len(real_lenghts)):
            real_lenghts[_] += suffix_len
        transformer_inputs = {
            'inputs_embeds': torch.stack(inputs_embeds, dim=0)*self.input_embed_scale, #B,T,D
            'attention_mask': attention_mask ,#attention_mask
            
        }
        return transformer_inputs,real_lenghts
    ...
    def forward(self,**kwargs):
        if self.input_type=='gloss':
            ...
            kwargs.pop('gloss_ids', None)
            kwargs.pop('gloss_lengths', None)
            new_kwargs,real_lenghts = self.prepare_feature_inputs(input_feature, input_lengths)
            kwargs = {**kwargs, **new_kwargs}
            real_input_lenghts = real_lenghts
            kwargs['input_lengths'] = real_input_lenghts
        ...
        output_dict = self.model(**kwargs, return_dict=True)
        encoder_mask = kwargs['attention_mask']
        if 'decoder_lastLayer_self_att' in output_dict:
            decoder_lastLayer_self_att = output_dict['decoder_lastLayer_self_att']
        else:
            decoder_lastLayer_self_att = None
        decoder_input_ids = kwargs['decoder_input_ids']
        log_prob = torch.nn.functional.log_softmax(output_dict['logits'], dim=-1)  # B, T, L
        batch_loss_sum = self.translation_loss_fun(log_probs=log_prob,targets=kwargs['labels'])
        if self.mycfg is not None and self.mycfg.get('debias_lm_head',None) is not None:
            multi_lm_logit = output_dict['multi_lm_logit']
            multi_lm_logit = torch.logsumexp(multi_lm_logit, dim = -1)
            multi_log_prob = torch.nn.functional.log_softmax(multi_lm_logit, dim = -1)
            multi_batch_loss_sum = self.translation_loss_fun(log_probs=multi_log_prob,targets=kwargs['labels'])
            output_dict['multi_translation_loss'] = multi_batch_loss_sum/multi_log_prob.shape[0]
        output_dict['translation_loss'] = batch_loss_sum/log_prob.shape[0]
        output_dict['translation_loss_sum'] = batch_loss_sum

        output_dict['transformer_inputs'] = kwargs #for later use (decoding)
        output_dict['encoder_mask'] = encoder_mask
        output_dict['decoder_input_ids'] = decoder_input_ids
        output_dict['inputs_embeds'] = new_kwargs['inputs_embeds']
        output_dict['real_input_lenghts'] =  real_input_lenghts
        return output_dict
